...
async def get_chat_history(
    session_id: UUID, user=Depends(verify_jwt), db: Session = Depends(get_db)
):
    # Retrieve the chat session and verify the user
    chat_session = (
        db.query(ChatSession)
        .filter(ChatSession.id == session_id, ChatSession.user_id == user.user.id)
        .first()
    )
    if not chat_session:
...
async def get_all_chat_sessions(
    user=Depends(verify_jwt), db: Session = Depends(get_db)
):
    chat_sessions = (
        db.query(ChatSession)
        .filter(ChatSession.user_id == user.user.id)
...
async def empty_chat_sessions_and_messages(db: Session = Depends(get_db)):
