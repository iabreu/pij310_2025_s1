      ...
      <section className="w-full h-full">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-5 w-full h-auto">
          <div className="w-full flex flex-col">
            ...
            <table className='w-fit flex text-left bg-zinc-700 rounded-md p-2'>
              <tbody>
                ...
                <tr className='border-b-2'>
                  <th className="w-[200px] text-left p-2">Data exame Inicial</th>
          ...
          <div className="w-full flex items-center gap-4 p-2">
            ...
            <span className={`${statusColor(patientData?.status || 'Unknown')} text-center w-fit h-fit px-2.5 py-0.5 rounded-full`}>{patientData?.status}</span>
          </div>
              ...
              <tbody>
                <tr>
                  {patientData?.case_histories.map((historie, key: number) => (
                    <td key={key} className='p-2'>{historie.diagnosis_date}</td>
                  ))}
                </tr>
              </tbody>
            </table>
